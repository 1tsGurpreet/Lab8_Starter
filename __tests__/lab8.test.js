describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadow = await firstProdItem.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let itr;
    for(itr = 1; itr < prodItems.length; ++itr){
      // check every element to make sure that all of their buttons say "Remove from Cart".
      let shadow = await prodItems[itr].getProperty('shadowRoot');
      let button = await shadow.$('button');
      let butText = await button.getProperty('innerText');
      
      if(butText['_remoteObject'].value != 'Remove from Cart'){
        itr = -1;
        break;
      }
    }
    ...
    const cart = await page.$('#cart-count');
    let cartCount = await cart.getProperty('innerText');
    expect(cartCount['_remoteObject'].value == '20' && itr != -1).toBe(true);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; ++i){
      ...
      if(butText['_remoteObject'].value == 'Add to Cart');
      else{
        remember = true;
        break;
      }
    ...
    const cart = await page.$('#cart-count');
    let cartCount = await cart.getProperty('innerText');
    expect(cartCount['_remoteObject'].value == '0' && !remember).toBe(true);
  }, 10000);
